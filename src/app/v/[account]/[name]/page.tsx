...
import { decryptVideoBytes, loadVideoKey } from "@/utils/videoCrypto";
...
export default function WatchPage() {
  ...
  const [uploaderProfile, setUploaderProfile] = useState<ProfileData | null>(null);
  ...
  // Load uploader profile for this video
  useEffect(() => {
    let cancelled = false;
    const load = async () => {
      if (!decodedAccount) {
      ...
      try {
        const p = await fetchProfile(decodedAccount);
        if (!cancelled) setUploaderProfile(p);
      } catch {
        if (!cancelled) setUploaderProfile(null);
      }
    };
    load();
    return () => {
  ...
  // If video was uploaded encrypted on this device, decrypt locally before playback
  useEffect(() => {
    let revokedUrl: string | null = null;
    let cancelled = false;

    const maybeDecrypt = async () => {
      if (!decodedAccount || !decodedName || !videoUrl) {
        setResolvedUrl(videoUrl);
        return;
      }
      const keyInfo = loadVideoKey(decodedAccount, decodedName);
      if (!keyInfo) {
        setResolvedUrl(videoUrl);
        return;
      }
      try {
        ...
        const plain = await decryptVideoBytes(buf, keyInfo.keyB64, keyInfo.ivB64);
        ...
        console.error("Failed to decrypt video, falling back to raw URL", e);
        if (!cancelled) {
          setResolvedUrl(videoUrl);
        }
      }
  ...
  }, [decodedAccount, decodedName, videoUrl]);
          ...
          <div className="mt-4 flex flex-wrap items-start justify-between gap-4 border-b border-white/10 pb-4">
            <div>
              ...
              <p className="mt-1 inline-flex items-center gap-1 text-sm font-semibold">
                <span className="bg-gradient-to-r from-[#ff4b8b] via-[#b544ff] to-[#00f5a0] bg-clip-text text-transparent">
                  {uploaderProfile?.channelName ??
                    `${decodedAccount.slice(0, 6)}…${decodedAccount.slice(-6)}`}
                </span>
              </p>
              <p className="mt-0.5 text-sm text-[#aaaaaa]">
